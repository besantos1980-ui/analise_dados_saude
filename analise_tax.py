...
def processar_contabeis():
    ...
    # Contas de interesse para este processamento
    contas_alvo = ["6111", "6112", "6119", "612"]
    ...
    for ano, t in candidatos:
        ...
        try:
            ...
            # Se a ANS não publicou ainda (404), pula e tenta o trimestre anterior
...
def main():
    ...
    # 2. Garante que todas as contas existam como coluna para evitar erros de cálculo
    contas_alvo = ["6111", "6112", "6119", "612"]
    for conta in contas_alvo:
        if conta not in df_pivot.columns:
            df_pivot[conta] = 0.0

    # 3. Realiza as somas e criações de colunas solicitadas
    df_pivot['impostos e contribuições'] = df_pivot['6111'] + df_pivot['6112'] + df_pivot['6119']
    df_pivot['Participações sobre o lucro'] = df_pivot['612']
    
    # 4. Cálculo do Resultado Final
    df_pivot['Resultado Final'] = df_pivot['impostos e contribuições'] + df_pivot['Participações sobre o lucro']

    # 5. Filtra e organiza as colunas finais
    colunas_finais = [
        'REG_ANS', 
        'Trimestre', 
        '6111',
    ...
    # 6. Converte REG_ANS para número estrito (Int64)
    ...
    # 7. Salva o resultado em Excel
    ts = datetime.today().strftime("%d_%m_%Y")
    out_file = f"base_impostos_participacoes_{ts}.xlsx"
